...
class AlproVideoTrainProcessor(AlproVideoBaseProcessor):
    ...
    def __call__(self, vpath):
        ...
        try:
            ...
            if clip.shape == (3, self.n_frms, self.image_size, self.image_size):
                return clip
            else:
                warnings.warn(f"Failed to load examples with video: {vpath}. Size mismatch.")
                return None
        except:
            warnings.warn(f"Failed to load examples with video: {vpath}. Transform error.")
            return None
