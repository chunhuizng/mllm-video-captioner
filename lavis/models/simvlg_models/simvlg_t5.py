...
@registry.register_model("simvlg_t5")
class SimvlgT5(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.t5_proj = nn.Linear(
            self.Tomeformer.config.hidden_size, self.t5_model.config.hidden_size
        )
    ...
    def forward(self, samples):
        if not self.scst: # xe training
            ...
            with self.maybe_autocast():
                encoded_images = self.visual_encoder(image)
                _, dim1, dim2 = encoded_images.shape
                encoded_images = encoded_images.reshape(B,T,dim1,dim2)
                ...
                tomeformer_output = self.Tomeformer(
                    inputs_embeds=self.vit_proj(image_embeds),
                    attention_mask=image_atts,
                    return_dict=True,
                )

                inputs_t5 = self.t5_proj(tomeformer_output.last_hidden_state)
